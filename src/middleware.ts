...
export async function middleware(req: NextRequest) {
  ...
  if (protectedPaths.includes(url.pathname)) {
    ...
    if (!accessToken || !(await checkAccessTokenValidity(accessToken))) {
      ...
      if (refreshToken) {
        console.log(
          "🔄 리프레시 토큰이 존재하므로 새로운 액세스 토큰 요청 시도"
        );
        if (await refreshAccessToken(refreshToken)) {
          console.log("✅ 리프레시 성공! 정상적으로 페이지 접근 가능");
          const endTime = performance.now();
          console.log(`🕒 미들웨어 실행 시간: ${endTime - startTime}ms`);
          return NextResponse.next();
        }
      }
      console.log(
        "🚫 리프레시 토큰 없음 또는 리프레시 실패 → 로그인 페이지로 리디렉트"
      );
...
const refreshAccessToken = async (refreshToken: string) => {
  ...
  try {
    ...
    console.log(
      `🔄 리프레시 요청 결과: ${res.status} ${res.ok ? "✅ 성공" : "❌ 실패"}`
    );
    return res.ok;
  } catch (error) {
